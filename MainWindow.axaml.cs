...
public partial class MainWindow : Window
{
    ...
    private void LoadIsland(string path)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                Dispatcher.UIThread.BeginInvoke(DispatcherPriority.Background, new Action(RenderSoftwareTerrain));
            }
            UpdateZoomLabel();
            ApplyMode();
        }
    ...
    private void ApplyInteriorView()
    {
        ...
        InteriorViewImage.RenderTransform = CompatTransforms.Matrix(interiorZoom, 0, 0, interiorZoom, vw / 2 - cx * interiorZoom, vh / 2 - cy * interiorZoom);
        RenderOptions.SetBitmapInterpolationMode(InteriorViewImage, interiorZoom >= 1 ? BitmapInterpolationMode.None : BitmapInterpolationMode.HighQuality);
    ...
    private void AddDummyMarker(double cx, double cy, double height)
    {
        ...
        image.Effect = new DropShadowEffect { Color = Colors.White, BlurRadius = 4, OffsetX = 0, OffsetY = 0, Opacity = 0.9 };
    ...
    private void AddBodyMarker(Bitmap image, double cx, double cy, double halfHeight)
    {
    ...
    private IBrush? savedMinimapBackground;
    private IImage? savedIslandMinimap;
    ...
    private void GameCombo_SelectionChanged(object? sender, SelectionChangedEventArgs e)
    {
        ...
        Dispatcher.UIThread.BeginInvoke(DispatcherPriority.Background, new Action(() => Keyboard.Focus(this)));
    }
    ...
    private void BuildSceneMinimap(Bitmap canvas, Rect content)
    {
        Interlocked.Increment(ref minimapRequest);
        ...
        Bitmap cropped = BitmapFactory.Crop(canvas, new PixelRect((int)content.X, (int)content.Y, (int)content.Width, (int)content.Height));
        var thumbnail = BitmapFactory.Scale(cropped, sceneMinimapScale, sceneMinimapScale);
        thumbnail.Freeze();
        MinimapImage.Source = thumbnail;
        MinimapActorCanvas.Children.Clear();
        foreach (var (_, x, y, _, _, _) in interiorActors)
        {
            var dot = new Avalonia.Controls.Shapes.Ellipse { Width = 4, Height = 4, Fill = Brushes.Yellow, Stroke = Brushes.Black, StrokeThickness = 0.5 };
    ...
    private void UpdateSceneMinimapViewport()
    {
        ...
        var box = new Avalonia.Controls.Shapes.Rectangle { Width = Math.Max(4, view.Width), Height = Math.Max(4, view.Height), Stroke = Brushes.Yellow, StrokeThickness = 1.5 };
    ...
    private void OpenLba1ActorWindow(int key, bool edit = false)
    {
        ...
        try
        {
            window = new Lba1ActorAttributesWindow(lba1Game, lba1Images, sceneNumber, actor.Index, title,
                edited => SaveLba1Actor(sceneNumber, edited), () => OpenLba1ScriptWindow(key)).WithOwner(this);
        }
    ...
    private void OpenLba1ScriptWindow(int key)
    {
        ...
        var window = new ActorScriptWindow(null, lba1Session, ResolveLba1Source, DescribeLba1Actor, () => { zoneCache.Clear(); ReloadLba1AfterEdit(); }).WithOwner(this);
    ...
    private void AddZoneShapes(IEnumerable<ProjectedZone> zones, Func<Point, Point> map, double width, double height)
    {
        foreach (var zone in zones)
        {
            ...
            IBrush brush = selected ? Brushes.White : new SolidColorBrush(color);
            var geometry = ZoneStyle.Wireframe(new ProjectedZone(zone.Type, zone.Num, pts), p => p);
            ActorMarkerCanvas.Children.Add(new Avalonia.Controls.Shapes.Path
            {
                Data = geometry,
                Stroke = brush,
            ...
            if (zone.Ref is not null)
            {
                ...
                hit.PointerPressed += ZoneShape_MouseLeftButtonDown;
    ...
    private void Lba2Editor_Click(object? sender, RoutedEventArgs e)
    {
        ...
        try { var w = new Lba2SceneEditorWindow(gameRoot, start, EditLba2ScriptFromEditor).WithOwner(this); WindowLifecycle.Register(w, "Lba2SceneEditorWindow"); w.ShowOwned(); }
        catch (Exception error) when (error is IOException or InvalidDataException or UnauthorizedAccessException or ArgumentException)
        {
    ...
    private void GridEditor_Click(object? sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            var gridWindow = new GridEditorWindow(Lba1Game.IsInstalled(lba1) ? lba1 : null, lba2, describe).WithOwner(this);
            WindowLifecycle.Register(gridWindow, "GridEditorWindow");
            gridWindow.ShowOwned();
        }
        catch (Exception error) when (error is IOException or InvalidDataException or UnauthorizedAccessException or ArgumentException)
        {
    ...
    private void ObjectBrowser_Click(object? sender, RoutedEventArgs e)
    {
        ...
        try { var w = new ObjectBrowserWindow(lba1, lba2).WithOwner(this); WindowLifecycle.Register(w, "ObjectBrowserWindow"); w.ShowOwned(); }
        catch (Exception error) when (error is IOException or InvalidDataException or UnauthorizedAccessException or ArgumentException)
        {
    ...
    private void AssetEditor_Click(object? sender, RoutedEventArgs e)
    {
        ...
        try { var w = new AssetEditorWindow(lba1, lba2).WithOwner(this); WindowLifecycle.Register(w, "AssetEditorWindow"); w.ShowOwned(); }
        catch (Exception error) when (error is IOException or InvalidDataException or UnauthorizedAccessException or ArgumentException)
        {
    ...
    private void EditLba2ScriptFromEditor(int scene, int actor)
    {
        ...
        var window = new ActorScriptWindow(null, scriptSession, k => new ActorSource((k - 1_000_000) / 1000, (k - 1_000_000) % 1000)).WithOwner(this);
        window.Closed += (_, _) => openScriptWindows.Remove(key);
        openScriptWindows[key] = window;
        window.ShowActor(key);
    }
    ...
    private void Lba1Editor_Click(object? sender, RoutedEventArgs e)
    {
        ...
        try
        {
            var sceneWindow = new Lba1SceneEditorWindow(directory, start, EditLba1ActorFromEditor).WithOwner(this);
            WindowLifecycle.Register(sceneWindow, "Lba1SceneEditorWindow");
            sceneWindow.ShowOwned();
        }
        catch (Exception error) when (error is IOException or InvalidDataException or UnauthorizedAccessException or ArgumentException)
        {
    ...
    private void DrawInteriorActorOverlay()
    {
        ...
        foreach (var (actorIndex, canvasPoints) in pathsVisible ? interiorOverlay.Routes : new())
        {
            ...
            ActorMarkerCanvas.Children.Add(new Avalonia.Controls.Shapes.Polyline
            {
                Points = points,
                Stroke = brush,
                StrokeThickness = 1.5,
        ...
        foreach (var (index, x, y, halfWidth, halfHeight, isMarker) in interiorActors)
        {
            ...
            if (lba2JoinedView) hit.PointerPressed += JoinedLba2Actor_MouseLeftButtonDown;
            else
            {
                hit.PointerPressed += ActorMarker_MouseLeftButtonDown;
                hit.PointerPressed += ActorMarker_MouseRightButtonDown;
            }
        ...
        for (var i = 0; i < count; i++)
        {
            ...
            if (pathsVisible && waypointCount > 0)
            {
                ...
                for (var w = 0; w < waypointCount; w++)
                {
                    ...
                    var waypointWorld = new Vec3(wx, wy, wz);
                ...
                if (points.Count > 1)
                {
                    var route = new Avalonia.Controls.Shapes.Polyline
                    {
                        Points = points,
                        Stroke = brush,
                        StrokeThickness = 1.5,
            ...
            dot.PointerPressed += ActorMarker_MouseLeftButtonDown;
            dot.PointerPressed += ActorMarker_MouseRightButtonDown;
    ...
    private void TerrainViewport_MouseRightButtonDown(object? sender, PointerEventArgs e)
    { if (!e.IsRight) return;
    ...
    private void ActorMarker_MouseLeftButtonDown(object? sender, PointerEventArgs e)
    { if (!e.IsLeft) return;
        if (((Control)sender).Tag is not int index) return;
        e.Handled = true;
        selectedActorIndex = index;
        RefreshActorOverlayForSelection();
    ...
    private void OpenActorScriptWindow(int index)
    {
        ...
        var window = new ActorScriptWindow(nativeRenderer.RendererLibrary, scriptSession, ResolveActorSource).WithOwner(this);
        WindowLifecycle.Register(window, "ActorScriptWindow");
    ...
    private void OpenActorAttributesWindow(int index, bool edit = false)
    {
        ...
        var window = new ActorAttributesWindow(nativeRenderer, palette, index).WithOwner(this);
    ...
    private void ActorMarker_MouseRightButtonDown(object? sender, PointerEventArgs e)
    { if (!e.IsRight) return;
        if (((Control)sender).Tag is not int index) return;
        e.Handled = true;
        selectedActorIndex = index;
    ...
    private void RegenerateMinimap()
    {
        ...
        Task.Run(() =>
        {
            ...
            var cropped = BitmapFactory.Crop(full, new PixelRect(offsetX, offsetY, cropWidth, cropHeight));
            cropped.Freeze();
    ...
    private void Minimap_MouseDown(object? sender, PointerPressedEventArgs e)
    { if (!e.IsLeft) return;
    ...
    private void TerrainViewport_MouseDown(object? sender, PointerPressedEventArgs e)
    {
    ...
    private void DrawActorMarkers()
    {
        ...
        for (var i = 0; i < count; i++)
        {
            ...
            var dot = new Avalonia.Controls.Shapes.Ellipse { Width = 5, Height = 5, Fill = brush, Stroke = Brushes.Black, StrokeThickness = 0.5 };
    ...
    // they belong to.
    private static Avalonia.Controls.Shapes.Path CreateRouteFlag(double x, double y, IBrush brush)
    {
        const double poleHeight = 8, pennantWidth = 5, pennantHeight = 4;
        var geometry = new PathGeometry();
        var poleFigure = new PathFigure { StartPoint = new Point(x, y), IsClosed = false, IsFilled = false };
        poleFigure.Segments!.Add(new LineSegment { Point = new Point(x, y - poleHeight) });
        geometry.Figures!.Add(poleFigure);
        var pennantFigure = new PathFigure { StartPoint = new Point(x, y - poleHeight), IsClosed = true };
        pennantFigure.Segments!.Add(new LineSegment { Point = new Point(x + pennantWidth, y - poleHeight + pennantHeight / 2) });
        pennantFigure.Segments!.Add(new LineSegment { Point = new Point(x, y - poleHeight + pennantHeight) });
        geometry.Figures.Add(pennantFigure);
        return new Avalonia.Controls.Shapes.Path { Data = geometry, Stroke = Brushes.Black, StrokeThickness = 0.5, Fill = brush };
    }
    ...
    private void MainWindow_KeyDown(object? sender, KeyEventArgs e)
    {
        ...
        if (Keyboard.Modifiers == KeyModifiers.Control && e.Key is Key.S or Key.O && Keyboard.FocusedElement is not TextBox)
        {
    ...
    {
        var dialog = new SettingsWindow().WithOwner(this);
